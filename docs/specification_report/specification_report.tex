...
\begin{figure}[p!]
  ...
  \begin{justify}
    ...
    software system. The \textbf{Register User} \usecaseall the system to register a new user. The 
    \textbf{Authenticate Student} \usecaseall the system to authenticate a student's credentials.
    The \textbf{Logout Student} \usecaseall the system to de-authenticate a student. The 
    \textbf{Calculate Concentration Progress} \usecaseall the system to generate the concentration
    requirement structure for a student. The \textbf{Manage Semesters} \usecaseall the frontend to
    manage semesters through the backend. The \textbf{Manage Courses} \usecaseall the frontend to
    manage courses through the backend. The \textbf{Manage Categories} \usecaseall the frontend to
    manage categories through the backend. The \textbf{Manage Grade Entries} \usecaseall the
    frontend to manage grade entries through the backend.
  \end{justify}
\end{figure}
...
\begin{figure}[p!]
  ...
  \caption{Level 1 DFD}
  \begin{flushleft}
    The level 1 DFD depicts the main use cases of the backend API. All but one of the main use cases
    is generic enough that the implementation is not worth decomposing further (already implemented
    by software libraries). In these use cases, the word \emph{manage} is referring to being able to
    create, retrieve, update and delete the data structure. The \emph{Calculate Concentration
    Progress} process is for constructing a recursive data structure representing the requirement
    structure of a concentration, including which requirements/courses the student has completed.
  \end{flushleft}
\end{figure}
...
\subsection{Authenticate Student}
\begin{algorithm}[H]
\newcommand{\request}{request}
\newcommand{\UserIsValid}{user\_is\_valid}

\SetKwFunction{AuthenticateStudent}{authenticate\_student}
\Fn{\AuthenticateStudent{\request}}{
  \KwData{HTTP request instance containing authentication credentials}

  \KwResult{Exception if credentials are invalid, else, auth token}
  \tcc{Validate the user. Throws exception if invalid}
  validate\_user($\request.data$)\\
  \tcc{Create and return token}
  \Return{\{$token$: create\_token($\request.user$)\}}
}
\end{algorithm}

\subsection{Retrieve Requirement}
\begin{algorithm}[H]

\SetKwFunction{RetrieveRequirement}{retrieve\_requirement}
\Fn{\RetrieveRequirement{concentration}}{
  \KwData{Concentration instance that the requirement structure belongs to}
  \KwResult{Root requirement of concentration}
  \tcc{Query requirement using the Django ORM}
  \Return{Requirement.objects.get($concentration\_id=concentration.id$)}
}
\end{algorithm}

\section{Conclusion}
With the completion of this project, I hope to have a major impact on other students' lives. After
using the product personally for tracking my own grades, I have experienced firsthand its immense
usefulness in increasing productivity. Over the course of the following years I hope to continually
expand this project, making it easier for students to use and potentially offering it as a paid
service. I believe that this product has lots of possibilities for improving college students lives
and I look forward to its continued progression.
